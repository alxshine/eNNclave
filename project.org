...
It reaches 66% (for some reason).
This is the output
#+include: "cluster_outputs/512_neurons_100_epochs.log"  src text

******* Fewer steps
I think the large number of training steps might limit our search of the weight space.
I will try to train a model with only 4 steps per epoch, but 1000 epochs.
