...
for i in range(max_index):
    if i < vgg16_len:
        vgg16_tf = f"{vgg16['tf_time'][i]:.02f}"
        
        if i > 0:
            vgg16_ee = f"{vgg16['enclave_time'][i]:.02f}"
            vgg16_ei = f"{vgg16['enclave_setup_time'][i]:.02f}"
        else:
            vgg16_ee = '--'
            vgg16_ei = '--'

        vgg16_total_time = vgg16['tf_time'][i] + vgg16['enclave_time'][i] + vgg16['enclave_setup_time'][i]
        vgg16_total = f"{vgg16_total_time:.02f}"
    else:
        vgg16_tf = '--'
        vgg16_ee = '--'
        vgg16_ei = '--'
    ...
    if i < vgg19_len:
        vgg19_tf = f"{vgg19['tf_time'][i]:.02f}"
        
        if i > 0:
            vgg19_ee = f"{vgg19['enclave_time'][i]:.02f}"
            vgg19_ei = f"{vgg19['enclave_setup_time'][i]:.02f}"
        else:
            vgg19_ee = '--'
            vgg19_ei = '--'

        vgg19_total_time = vgg19['tf_time'][i] + vgg19['enclave_time'][i] + vgg19['enclave_setup_time'][i]
        vgg19_total = f"{vgg19_total_time:.02f}"
    else:
        vgg19_tf = '--'
        vgg19_ee = '--'
        vgg19_ei = '--'
    ...
    if i < text_len:
        text_tf = f"{text['tf_time'][i]:.02f}"
        
        if i > 0:
            text_ee = f"{text['enclave_time'][i]:.02f}"
            text_ei = f"{text['enclave_setup_time'][i]:.02f}"
        else:
            text_ee = '--'
            text_ei = '--'

        text_total_time = text['tf_time'][i] + text['enclave_time'][i] + text['enclave_setup_time'][i]
        text_total = f"{text_total_time:.02f}"
    else:
        text_tf = '--'
        text_ee = '--'
        text_ei = '--'
