import os
import numpy as np

from enclave import Enclave
from enclave_layer import EnclaveLayer
import interop.pymatutil as pymatutil

import tensorflow.keras as keras
from tensorflow.keras.datasets import mnist
from tensorflow.keras.models import Sequential, load_model
from tensorflow.keras.layers import Dense, Flatten, Dropout
from tensorflow.keras.layers import Conv2D, MaxPooling2D
from tensorflow.keras import backend as K

batch_size = 128
num_classes = 10
epochs = 1
model_file = 'models/tf_enclave_mnist_cnn.h5'

img_rows, img_cols = 28, 28

input_shape = (img_rows, img_cols, 1)
num_classes = 10
logdir = "logs/"

(x_train, y_train), (x_test, y_test) = mnist.load_data()

if K.image_data_format() == 'channels_first':
    x_train = x_train.reshape(x_train.shape[0], 1, img_rows, img_cols)
    x_test = x_test.reshape(x_test.shape[0], 1, img_rows, img_cols)
    input_shape = (1, img_rows, img_cols)
else:
    x_train = x_train.reshape(x_train.shape[0], img_rows, img_cols, 1)
    x_test = x_test.reshape(x_test.shape[0], img_rows, img_cols, 1)
    input_shape = (img_rows, img_cols, 1)

x_train = x_train.astype('float32')
x_test = x_test.astype('float32')
x_train /= 255
x_test /= 255
print('x_train shape:', x_train.shape)
print(x_train.shape[0], 'train samples')
print(x_test.shape[0], 'test samples')

# convert class vectors to binary class matrices
y_train = keras.utils.to_categorical(y_train, num_classes)
y_test = keras.utils.to_categorical(y_test, num_classes)

if os.path.exists(model_file):
    print('Model found, loading from %s' % model_file)
    model = load_model(model_file, custom_objects={'Enclave': Enclave})
else:
    print('No model found, creating')
    enclave = Enclave()
    enclave.add(Dense(128, activation='relu'))
    enclave.add(Dropout(0.5))
    enclave.add(Dense(num_classes, activation='softmax'))

    model = Sequential()
    model.add(Conv2D(32, kernel_size=(3, 3),
                     activation='relu',
                     input_shape=input_shape))
    model.add(Conv2D(64, (3, 3), activation='relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Dropout(0.25))
    model.add(Flatten())
    model.add(enclave)

    model.compile(loss=keras.losses.categorical_crossentropy,
                  optimizer=keras.optimizers.Adadelta(),
                  metrics=['accuracy'])

    model.fit(x_train, y_train,
              batch_size=batch_size,
              epochs=epochs,
              verbose=1,
              validation_data=(x_test, y_test))

    print('Storing model at %s' % model_file)
    model.save(model_file)

# score = model.evaluate(x_test, y_test, verbose=0)
# print('Test loss:', score[0])
# print('Test accuracy:', score[1])
# print(model.summary())
# print(enclave.summary())

pymatutil.initialize()

enclave_model = Sequential(layers=model.layers[:5])
enclave_model.add(EnclaveLayer(10))

num_samples = 150
start_index = np.random.randint(0, x_test.shape[0]-num_samples)
test_set = x_test[start_index:start_index+num_samples]

# both results are vectors of probabilities (sort of)
print("Testing enclave on %d samples, starting from %d" %
      (num_samples, start_index))
expected_results = model.predict(test_set)
enclave_results = enclave_model.predict(test_set)

expected_results = np.argmax(expected_results, 1)
enclave_results = np.argmax(enclave_results, 1)
correct = np.array_equal(expected_results, enclave_results)

if correct:
    print("Enclave results are CORRECT (identical with regular model)")
else:
    print("Enclave results are INCORRECT")

pymatutil.teardown()
