...
****** TODO Use ~tf.py_func~ to call C code for testing
...
Actually, with TF as backend this still works (see [[file:python/tf_mnist.py::return%20tf.py_function(func=external_func,%20inp=%5Bx%5D,%20Tout=tf.float32)][this code]]).
...
****** TODO Extract weights, biases, etc.
I should do this for TF first, then Keras, then PyTorch.
