...
***** TODO Write extension to Keras Sequential [5/5]
...
**** TODO Make content of ~dense()~ and network weights independent of launch code
This would require decrypting the models and dynamically generating the ~dense()~ function.
However, this would allow signing one single enclave and then moving ML-as-a-service networks to the user, while still only giving them access to an API.
Not sure if this is too close to business applications, and maybe we want Google or Amazon to do this themselves.
...
*** DONE Find out if our processors have the foreshadow mitigation      :sgx:
